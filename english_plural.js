...
// Get the user-selected number of questions from localStorage.
// If not set, default to all available quiz items.
...
function showQuestion() {
  ...
  // Update and display the question count ("Question X of Y")
  questionCountElem.textContent = `Question ${currentIndex + 1} of ${totalQuestions}`;
  
  // Set and display the quiz word (with first letter capitalized)
...
function clearFeedback() {
  document.querySelectorAll(".explanation-text, .interactive-btn:not(.back-btn), p[style]").forEach(elem => {
    // Skip fixed elements: questionCount and quiz word (questionElem)
...
// Load the current question or show the final result if finished
